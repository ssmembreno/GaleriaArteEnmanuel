<section class="contact-section container py-5">
    <div class="contact-card p-4 rounded-bottom-5 bg-white position-relative">
        <h3 class="contact-title">{{__('messages.CONTACT_CONTACT')}}</h3>
        <div class="row">
            ...
            <div class="col-md-6">
                <form id="contact-form" action="{{route('contactanos.store')}}" method="POST">
                    ...
                    <div class="form-row d-flex gap-3 mb-3">
                        <div class="flex-fill">
                            <label for="firstName">{{__('messages.CONTACT_FORM_NAME')}}<span class="text-danger">*</span></label>
                            <input type="text" name="nombre" id="firstName" class="form-control" required>
                        </div>
                        <div class="flex-fill">
                            <label for="lastName">{{__('messages.CONTACT_FORM_APELLIDOS')}}<span class="text-danger">*</span></label>
                            <input type="text" name="apellido" id="lastName" class="form-control" required>
                        </div>
                    </div>

                    <div class="mb-3">
                        <label for="email">{{__('messages.CONTACT_FORM_EMAIL')}}<span class="text-danger">*</span></label>
                        <input type="email" name="email" id="email" class="form-control" required>
                    </div>

                    <div class="mb-3">
                        <label for="message">{{__('messages.CONTACT_FORM_MENSAJE')}}<span class="text-danger">*</span></label>
                        <textarea id="message" name="mensaje" class="form-control" rows="4" required></textarea>
                    </div>

                    <button type="submit" class="btn contact-form">{{__('messages.CONTACT_FORM_BTN')}}</button>
                </form>
